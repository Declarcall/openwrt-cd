...
## 16. 官方正规规范优先与绝禁应急硬补丁铁律 (Canonical Official Approach Priority Rule)
- **优先查探官方正规解法**：遇到任何编译错误、内核配置异常或依赖冲突时，**必须第一时间追查 OpenWrt 官方体系与架构规范中的正规解决途径（如 target 层 config 模板选单完全对齐）**。
- **绝禁使用应急硬补丁掩盖报错**：绝对严禁图省事在 Makefile 源码层或 Workflow 中注入 `sed` 强行篡改、加 `oldconfig` 管道或绕过报错的“非正规应急硬补丁”。所有的修复必须在源头架构层按照官方正规标准优雅解决！
