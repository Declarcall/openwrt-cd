...
if [[ "${WRT_TARGET^^}" == *"QUALCOMMAX"* ]] || [ -d "./target/linux/qualcommax" ]; then
	...
	if [ -f "./include/kernel-build.mk" ]; then
		...
		sed -i -E '/cmp -s.*\.config\.set/s/.*/\tcp \$(LINUX_DIR)\/\.config\.set \$(LINUX_DIR)\/\.config \&\& \$(KERNEL_MAKEOPTS) olddefconfig \&\& cp \$(LINUX_DIR)\/\.config \$(LINUX_DIR)\/\.config\.prev/' ./include/kernel-build.mk 2>/dev/null || true
	fi
